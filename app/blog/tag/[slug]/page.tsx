...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!tag) return {}
  return {
    title: `#${tag.name} | Blog NDS Kreasi`,
  ...
  return (
    <>
      <section className="py-16 bg-primary">
        <Container>
          ...
          <div className="max-w-2xl">
            ...
            <h1 className="text-4xl font-extrabold text-white mb-3">#{tag.name}</h1>
            <p className="text-white/50 text-sm mt-2">{published.length} artikel</p>
          </div>
        </Container>
      </section>
      ...
      <section className="py-16 bg-gray-50">
        <Container>
          {published.length === 0 ? (
            <p className="text-center text-gray-500 py-20">Belum ada artikel dengan tag ini.</p>
          ) : (
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-200 h-full flex flex-col overflow-hidden">
                    ...
                    <div className="p-6 flex flex-col flex-1">
                      {article.category ? (
                        ...
                        <span className="inline-block bg-accent/10 text-accent text-xs font-bold px-2.5 py-1 rounded-full mb-3 self-start">
                          #{tag.name}
                        </span>
                      )}
